...
-- v2.0.0: RETE and Forward Chaining explicit functions
...
-- Partial index using CURRENT_TIMESTAMP removed because CURRENT_TIMESTAMP is
-- not IMMUTABLE. Use a regular index instead so queries comparing expires_at
-- to the current time can still benefit from indexing.
...
-- ============================================================================
-- v1.7.0: Built-in Functions Library
-- ============================================================================
...
COMMENT ON FUNCTION rule_function_call IS 
'Execute a built-in function from SQL. 
Example: SELECT rule_function_call(''Round'', ''[3.14159, 2]''::jsonb);';
...
COMMENT ON FUNCTION rule_function_list IS 
'List all 24 built-in functions available in GRL rules.
Functions work in when conditions via preprocessing.';
-- NATS Integration (v1.8.0)
...
-- Note: The following functions are registered via pgrx and don't need SQL definitions:
-- - run_rule_engine(facts_json TEXT, rules_grl TEXT) -> TEXT
--   Now uses RETE algorithm by default (2-24x faster)
--
-- - run_rule_engine_rete(facts_json TEXT, rules_grl TEXT) -> TEXT
--   Explicit RETE execution for high-performance scenarios
--
-- - run_rule_engine_fc(facts_json TEXT, rules_grl TEXT) -> TEXT
--   Traditional forward chaining for simple cases
